/* Madeline Follosco
 * Due: 6/11/23
 * This file contains: functions to create and maintain a hash chaining implementation. Doubly and circularly linked lilists are used.
 */
...
//function: hash - the age and number of indexes in the array (number of ages in age range) are passed in to return a hash value that becomes the index of the a
//rray of structs where the data is located.
//time complexity: O(1)
...
//function: insert - a set pointer, random age and id are generated to be passed in. Depending on the hash value of the age the node is created and inserted at the right index.
//time complexity: O(1)
...
//function: deletion - a pointer to the set is passed in as well as the desired index of deletion. All the student  records of that age/index of the array of sets are deleted and all nodes are freed.
//time complexity: O(n)
